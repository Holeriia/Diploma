...
public class ParticipantAnalyticsView extends StandardListView<Participant> {
    ...
    @Supply(to = "analyticsDataGrid.topicsColumn", subject = "renderer")
    private Renderer<Participant> topicsColumnRenderer() {
        return new ComponentRenderer<>(p -> {
            Span span = new Span();
            if (p.getTopics() != null && !p.getTopics().isEmpty()) {
                String topicsText = p.getTopics().stream()
                        .map(Topic::getName)
                        .collect(Collectors.joining("; "));
                span.setText(topicsText);
            } else {
                span.setText("Темы не предложены");
                span.getStyle().set("color", "var(--lumo-secondary-text-color)");
