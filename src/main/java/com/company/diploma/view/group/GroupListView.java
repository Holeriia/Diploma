package com.company.diploma.view.group;
...
public class GroupListView extends StandardListView<Group> {
    ...
    @ViewComponent
    private InstanceContainer<Group> groupDc;
    ...
    @ViewComponent
    private DataGrid<Group> groupsDataGrid;
    ...
    public void onInit(final InitEvent event) {
        groupsDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("groupsDataGrid.createAction")
    public void onGroupsDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Group entity = dataContext.create(Group.class);
        groupDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("groupsDataGrid.editAction")
    public void onGroupsDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    public void onGroupsDcItemChange(final InstanceContainer.ItemChangeEvent<Group> event) {
        prepareFormForValidation();

        Group entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            groupDl.setEntityId(EntityValues.getId(entity));
            groupDl.load();
        } else {
            groupDl.setEntityId(null);
            groupDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private ValidationErrors validateView(Group entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        groupsDataGrid.getActions().forEach(Action::refreshState);
