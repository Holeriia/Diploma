package com.company.diploma.view.position;

import com.company.diploma.entity.Position;
...
public class PositionListView extends StandardListView<Position> {
    ...
    @ViewComponent
    private InstanceContainer<Position> positionDc;

    @ViewComponent
    private InstanceLoader<Position> positionDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<Position> positionsDataGrid;
    ...
    public void onInit(final InitEvent event) {
        positionsDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("positionsDataGrid.createAction")
    public void onPositionsDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Position entity = dataContext.create(Position.class);
        positionDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("positionsDataGrid.editAction")
    public void onPositionsDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    public void onPositionsDcItemChange(final InstanceContainer.ItemChangeEvent<Position> event) {
        prepareFormForValidation();

        Position entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            positionDl.setEntityId(EntityValues.getId(entity));
            positionDl.load();
        } else {
            positionDl.setEntityId(null);
            positionDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        Position item = positionDc.getItem();
    ...
    private ValidationErrors validateView(Position entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        positionsDataGrid.getActions().forEach(Action::refreshState);
