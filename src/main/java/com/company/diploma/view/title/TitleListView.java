package com.company.diploma.view.title;

import com.company.diploma.entity.Title;
...
@Route(value = "titles", layout = MainView.class)
@ViewController(id = "Title.list")
@ViewDescriptor(path = "title-list-view.xml")
@LookupComponent("titlesDataGrid")
@DialogMode(width = "64em")
public class TitleListView extends StandardListView<Title> {

    @ViewComponent
    private DataContext dataContext;

    @ViewComponent
    private CollectionContainer<Title> titlesDc;

    @ViewComponent
    private InstanceContainer<Title> titleDc;

    @ViewComponent
    private InstanceLoader<Title> titleDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<Title> titlesDataGrid;
    ...
    public void onInit(final InitEvent event) {
        titlesDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("titlesDataGrid.createAction")
    public void onTitlesDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Title entity = dataContext.create(Title.class);
        titleDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("titlesDataGrid.editAction")
    public void onTitlesDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "titlesDc", target = Target.DATA_CONTAINER)
    public void onTitlesDcItemChange(final InstanceContainer.ItemChangeEvent<Title> event) {
        prepareFormForValidation();

        Title entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            titleDl.setEntityId(EntityValues.getId(entity));
            titleDl.load();
        } else {
            titleDl.setEntityId(null);
            titleDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private ValidationErrors validateView(Title entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        titlesDataGrid.getActions().forEach(Action::refreshState);
